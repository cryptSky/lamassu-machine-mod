...
//const accountCreate = require('./flows/account-create')
...
Brain.prototype.processAction = function processAction (action) {
  ...
  switch (action) {
    ...
    case 'sanctionsFailure':
      ...
      break
    // below are account create flow actions
    //case 'timeoutToContactData':
    //  this.timeoutToContactData()
    //  break
    //case 'transitionScreenAccount':
    //  this.transitionScreenAccount()
    //  break
    //case 'saveContactData':
    //  this.saveContactData()
    //  break
//	case 'saveAddresIdData':
  //    this.saveAddressIdData()
  //    break
  //  case 'scanFrontPDF':
  //    this.scanFrontPDF()
  //    break
  ...
  //    this.scanBackPDF()
  //    break
  //  case 'timeoutToScanFrontPDFCancel':
  //    this.timeoutToScanFrontPDFCancel()
  //    break
  //  case 'timeoutToScanBackPDFCancel':
  //    this.timeoutToScanBackPDFCancel()
  //    break
  //  case 'authorizeAccountData':
  //    this.authorizeAccountData()
  //    break
  //  case 'timeoutToFailAccount':
  //    setTimeout(() => accountCreate.dispatch('FAIL'), 30000)
  //    break
  //  case 'successAccount':
  //    this.smsFlowHandleReturnState()
  //    break
...
Brain.prototype._processReal = function _processReal (req) {
  switch (req.button) {
    ...
    case 'cancelWifiPass':
      ...
      break
    case 'fundingSourceSelect':
      this._fundingSourceSelect(req.data)
      break
    case 'cancelFundingSourceList':
      this._cancelFundingSourceList()
      break
    case 'initialize':
      this.initialize()
      break
    ...
    case 'smsCompliance':
      const returnState = this.tx.fiat.eq(0)
        ? 'acceptingFirstBill'
        : 'acceptingBills'
      if (req.data != undefined && req.data.type == "account") {
          this.smsCompliance({returnState: returnState, type: "account"})
      }
      else 
      {
...
Brain.prototype._cancelFundingSourceList = function _cancelFundingSourceList () {
...
Brain.prototype.registerCompliance = function registerCompliance () {
...
Brain.prototype.smsCompliance = function smsCompliance (opts = {}) {
  ...
  const flow = new sms.Flow({noCode: opts.noCode, isMT: opts.type == "account"})
  this.flow = flow

  flow.on('screen', rec => {
    if (opts.type == "account") {
      this._transitionState(rec.screen, {context: 'account-compliance'})
   } else {
  ...
  flow.on('sendCode', phone => {
     this.trader.phoneCode(phone.phone, opts.type == 'account')
        .then(result => {

          if (opts.type == 'account') {
            if (!result.verified) {
              return flow.handle('phoneNumberNotRegistered')
            }

            var amount = this.complianceAmount() - 1
            var btm_tier = 1
            ...
            var tiers = []
            if (btm_tier - result.tier == 2) {
              tiers = [2, 3]
            } else if (btm_tier - result.tier == 1) {
              tiers = [btm_tier]
            }

            this.flow.tiers = tiers
            this.mt_name = result.mt_name
          }
  ...
  flow.on('sendFundingSource', data => {
    this.trader.sendFundingSource(data.phone, data.fundingSource)
      .then(result => {
          if (flow.tiers.length == 2) {
            return flow.handle('waitForSSN')
          } else {
            return flow.handle('success')
          }
      })
      .catch(err => {
         ...
         */
        this.networkDown = true
        this.smsFlowHandleReturnState(true)
      })
      .catch(err => {
        this.flow = null
        this._fiatError(err)
  ...
  flow.on('sendSSN', data => {
    this.trader.sendSSN(data.phone, data.ssn)
      .then(result => {
          return flow.handle('success')
      })
      .catch(err => {
  ...
  flow.on('success', () => {
    ...
    return txPromise
      .then(tx => {
        this.tx = tx
        var isMtFlow = opts.type == "account"
        return this.smsFlowHandleReturnState(isMtFlow)
      })
      .catch(err => {
...
Brain.prototype.isTierCompliant = function isTierCompliant (tier) {
  ...
  switch (tier) {
    ...
    case 'hardLimit':
      ...
      return true
    case 'idCardData':
      return (this.mt_name !== null && this.mt_name !== '') ? true : !_.isEmpty(customer.idCardData)
    case 'sanctions':
...
Brain.prototype.smsFlowHandleReturnState = function smsFlowHandleReturnState (isMT = false) {
  const returnState = this.returnState
  const tx = this.tx
  const amount = this.complianceAmount()
  var nonCompliantTiers = this.nonCompliantTiers(amount)

  if (isMT) {
    nonCompliantTiers.splice( nonCompliantTiers.indexOf('idCardData'), 1 );
    //this.nonCompliantTiers()
  }
  ...
  var data = null
  if (isMT && (returnState == 'acceptingFirstBill' || returnState == 'acceptingBills')) {
    data = {name: this.mt_name}
  }

  this._transitionState(returnState, data)
}
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  if (!this.isCompliant(amount)) {
   ...
   // return this._transitionState('accountCreate', {
   //   threshold: this.trader.smsVerificationThreshold
   // })
